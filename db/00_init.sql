...
create table if not exists public.funcoes (
  ...
  nome text not null,
  faccao_id bigint references public.faccoes(faccao_id)
);
...
create table if not exists public.membros (
  ...
  alcunha text,
  faccao_id bigint references public.faccoes(faccao_id),
  funcao_id bigint references public.funcoes(funcao_id)
);

-- seed simples
insert into public.faccoes (nome) values ('Alfa'), ('Bravo') on conflict do nothing;

insert into public.funcoes (nome, faccao_id) values
  ('Líder', 1), ('Operacional', 1), ('Líder', 2)
on conflict do nothing;

insert into public.membros (nome_completo, alcunha, faccao_id, funcao_id) values
...
base_nodes as (
  select 
    ...
    (case
      when coalesce(nullif(m.alcunha, ''), nullif(m.nome_completo, '')) is null
        then ('ID ' || m.membro_id::text)
      when nullif(m.alcunha, '') is not null
        then m.alcunha
  ...
  union all
  select f.faccao_id::text as id,
         coalesce(nullif(f.nome,''), 'Facção '||f.faccao_id::text)::text as label,
         'faccao'::text as type,
         f.faccao_id, null
  ...
  union all
  select fu.funcao_id::text as id,
         coalesce(nullif(fu.nome,''), 'Função '||fu.funcao_id::text)::text as label,
         'funcao'::text as type,
         fu.faccao_id, fu.funcao_id
