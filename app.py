...
# - live/health/ready/ops_status: sondas e status operacional
# - graph_membros: retorna grafo (nós/arestas) via Supabase RPC (fallback com e sem p_)
# - vis_visjs: página HTML com vis-network (arestas ultrafinas; busca; cores CV/PCC/funções; física OFF após estabilizar; toolbar estilizada pelo vis-style.css)
# - vis_pyvis: página HTML com PyVis (arestas ultrafinas; física OFF após estabilizar; busca; toolbar estilizada pelo vis-style.css)
# - /docs: Swagger UI custom usando /openapi.json do FastAPI
# - Utilidades: normalização de labels PG array, cache Redis, truncamento seguro
# =============================================================================
...
# -----------------------------------------------------------------------------
# Endpoint VISJS (corrigido para usar CSS centralizado)
# -----------------------------------------------------------------------------
@app.get("/v1/vis/visjs", response_class=HTMLResponse, tags=["viz"])
async def vis_visjs(
    response: Response,
...
<html lang="pt-br">
  ...
  <body data-theme="{theme}">
    <div class="kg-toolbar">
      ...
      <button id="btn-reload">Recarregar</button>
    </div>
    <div id="mynetwork" style="height:90vh;width:100%;" data-source="server"></div>
    {embedded_block}
    <script src="{js_href}"></script>
    <script>
      const container=document.getElementById('mynetwork');
      const data=JSON.parse(document.getElementById('__KG_DATA__').textContent);
      ...
      const options={{
        ...
        edges:{{ width:0.1, arrows:{{ to:{{enabled:true, scaleFactor:0.5}} }} }},
        nodes:{{ shape:'dot', borderWidth:1 }}
      }};
      const net=new vis.Network(container,{{nodes:dsNodes,edges:dsEdges}},options);
