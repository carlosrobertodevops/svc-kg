...
async def supabase_rpc_get_graph(faccao_id: Optional[int], include_co: bool, max_pairs: int) -> Dict[str, Any]:
    ...
    # Chame com p_* apenas (evita 404 do PostgREST em functions nomeadas)
    payload = {
    ...
    log.info("HTTP Request: POST %s %s", url, resp.status_code)
...
# -----------------------------------------------------------------------------
# VIS.JS (vis-network) — HTML seguro (sem .format em strings com chaves)
# -----------------------------------------------------------------------------
@app.get(
    "/v1/vis/visjs",
...
async def vis_visjs(
    ...
    try:
        ...
        # Template com marcadores exclusivos (sem usar .format / f-string)
        template = r"""
<!doctype html>
<html lang="pt-br">
  <head>
...
"""
        ...
        log.exception("vis_visjs error: %s", e)
        return HTMLResponse(content=f"<pre>visjs error: {e}</pre>", status_code=500)


# -----------------------------------------------------------------------------
# PYVIS (set_options com JSON válido)
# -----------------------------------------------------------------------------
@app.get(
    "/v1/vis/pyvis",
    response_class=HTMLResponse,
...
async def vis_pyvis(
    ...
    try:
        ...
        # >>>>>> CORREÇÃO: set_options com JSON válido (não JS)
        options = {
            "interaction": {
                "hover": True,
                "dragNodes": True,
        ...
        # Barra de ferramentas (busca/print/reload)
        toolbar_css = """
        <style>
          .kg-toolbar { display:flex; gap:8px; align-items:center; padding:8px; border-bottom:1px solid #e0e0e0; }
          .kg-toolbar input[type="search"] { flex: 1; min-width: 220px; padding:6px 10px; }
        ...
        log.exception("vis_pyvis error: %s", e)
        return HTMLResponse(content=f"<pre>pyvis error: {e}</pre>", status_code=500)
