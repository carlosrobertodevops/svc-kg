...
# - graph_membros: retorna grafo (nós/arestas) via Supabase RPC (fallback com e sem p_)
# - vis_visjs: página HTML com vis-network (sem f-string no JS; arestas ultrafinas; busca; cores CV/PCC/funções; física OFF após estabilizar)
# - vis_pyvis: página HTML com PyVis (arestas ultrafinas; física OFF após estabilizar; busca)  [ajustada nesta versão p/ melhor separação]
# - /docs: Swagger UI custom usando /openapi.json do FastAPI
# - Utilidades: normalização de labels PG array, cache Redis, truncamento seguro
# Atualização: 08/09/2025 17h51min
# =============================================================================
...
async def vis_pyvis(
    ...
    def color_from_faccao(fid: Optional[str]) -> Optional[str]:
        ...
        if not name:
            return None
        if "PCC" in name:
            return "#0d47a1"  # azul (PCC)
        if "CV" in name:
            return "#d32f2f"  # vermelho (CV)
        return None
    ...
    # 3) degree → tamanho/massa (abre espaço entre clusters)
    node_ids_raw = {str(n["id"]) for n in nodes if n and n.get("id") is not None}
    deg: Dict[str, int] = {nid: 0 for nid in node_ids_raw}
    for e in edges:
        ...
        """
{
  ...
  "edges": {
    ...
    # 8) Toolbar + JS de busca/zoom/physics-off
    toolbar_css = """
<style>
  .kg-toolbar { display:flex; gap:8px; align-items:center; padding:8px; border-bottom:1px solid #e0e0e0; }
...
(function(){
  ...
  function runSearch(txt){
    try{
      ...
      all.forEach(function(n){ ds.update({ id: n.id, color: colorObj(n.color, 0.18) }); });
  ...
  if (typeof network !== 'undefined'){
    ...
    network.on('zoom', function (p) {
      var show = p.scale > 0.7;
      network.setOptions({ nodes: { font: { size: show ? 12 : 0 } } });
    });
  }
})();
</script>
