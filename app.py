...
BACKEND_MODE = (
    ...
    else ("postgres" if DATABASE_URL else "none")
)
...
async def _startup():
    ...
    log.info(
        f"Starting svc-kg v{app.version} | mode={BACKEND_MODE} | port={PORT} | supabase={SUPABASE_URL}"
    )
...
def truncate_preview(data: dict, max_nodes: int, max_edges: int) -> dict:
    ...
    keep = {
        n["id"]
        for n in sorted(nodes, key=lambda n: deg.get(n["id"], 0.0), reverse=True)[
            :max_nodes
        ]
    }
...
def _wrap_toolbar(
    html: str, title: str = "PyVis Graph", show_print_btn: bool = True
) -> str:
...
def _ensure_network_min_height(html: str, min_height: str = "90vh") -> str:
    ...
    if marker in html and 'id="mynetwork" style=' not in html:
        html = html.replace(
            marker, f'{marker} style="height:{min_height}; width:100%;"', 1
        )
    return html
...
async def build_pyvis_html(
    ...
    for n in nodes:
        ...
        size = n.get("size")
        if size is None:
            size = 10 + math.log(deg.get(nid, 0) + 1) * 8
        color = _hash_color(group)
        title_node = f"<b>{label}</b><br>id: {nid}<br>grupo: {group}<br>tipo: {ntype}"
        net.add_node(
            nid, label=label, title=title_node, color=color, size=size, shape="dot"
        )
    for e in edges:
        s = e.get("source")
        t = e.get("target")
...
async def build_pyvis_html_repo_style(
    ...
    for n in G.nodes():
        ...
        size = attrs.get("size")
        if size is None:
            size = sizes.get(n, 12.0)

        color = (
            hash_color(f"c{comm_map[n]}") if n in comm_map else hash_color(str(group))
        )
        title_node = f"<b>{label}</b><br>id: {n}<br>grupo: {group}<br>tipo: {ntype}"
        net.add_node(
            n, label=label, title=title_node, color=color, size=size, shape="dot"
        )
    ...
    for s, t, ed in G.edges(data=True):
        net.add_edge(
            ...
            t,
            title=ed.get("title"),
            value=float(ed.get("weight", 1.0) or 1.0),
            arrows="to" if arrows else "",
            physics=physics,
        )
...
# ------------------ backends ------------------
async def fetch_graph_via_supabase(
    ...
    if r.status_code >= 400:
        raise HTTPException(
            status_code=502,
            detail=f"Supabase RPC {SUPABASE_RPC_FN} falhou ({r.status_code}): {r.text[:400]}",
        )
    try:
...
async def fetch_graph_via_pg(
    ...
    async with pool.connection() as conn:
        async with conn.cursor() as cur:
            try:
                await cur.execute(
                    "select public.et_graph_membros(%s,%s,%s);",
...
async def fetch_graph_raw(
    ...
    raise HTTPException(
        status_code=500,
        detail="Nenhum backend configurado (defina SUPABASE_* ou DATABASE_URL)",
    )
    ...
    if BACKEND_MODE == "supabase":
        ...
        try:
            ...
            await asyncio.wait_for(
                loop.run_in_executor(None, socket.getaddrinfo, host, 443), timeout=2
            )
            info["dns_ok"] = True
        except Exception as e:
            info["dns_ok"] = False
...
async def health(response: Response):
    ...
    return {
        ...
        "env": APP_ENV,
        "cache": (
            "redis" if (ENABLE_REDIS_CACHE and REDIS_URL and redis_client) else "memory"
        ),
...
async def graph_membros(
    ...
    if data is None:
        data = await fetch_graph_sanitized(
            faccao_id, include_co, max_pairs, use_cache=True
        )
        if cache:
            await cache_set(key, data, CACHE_API_TTL)
...
async def graph_members_compat(
    ...
    if data is None:
        data = await fetch_graph_sanitized(
            p_faccao_id, p_include_co, p_max_pairs, use_cache=True
        )
        if cache:
            await cache_set(key, data, CACHE_API_TTL)
    out = truncate_preview(data, max_nodes, max_edges)
...
async def neighbors(
    response: Response, node_id: str, include_co: bool = True, max_pairs: int = 3000
):
    ...
    for e in edges:
        if e.get("source") == node_id:
            keep.add(e.get("target"))
        if e.get("target") == node_id:
            keep.add(e.get("source"))
...
@app.get(
    ...
    summary="Visualização PyVis (inline JS)",
)
async def vis_pyvis(
    response: Response,
    faccao_id: Optional[int] = Query(default=None),
...
<body><div class="empty">Nenhum dado para exibir (nodes=0).</div></body></html>"""
        if allow_inline:
            response.headers["Content-Security-Policy"] = (
                "default-src 'self' data: blob:; style-src 'self' 'unsafe-inline' data: blob:; script-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; img-src * data: blob:; font-src 'self' data:; connect-src *;"
            )
    ...
    if toolbar:
        html = _wrap_toolbar(html, title=title, show_print_btn=True)
    if debug:
        html = _append_debug_overlay(html, len(out["nodes"]), len(out["edges"]))
    if allow_inline:
        response.headers["Content-Security-Policy"] = (
            "default-src 'self' data: blob:; style-src 'self' 'unsafe-inline' data: blob:; script-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob:; img-src * data: blob:; font-src 'self' data:; connect-src *;"
        )
...
async def vis_visjs(
    ...
    if source == "server":
        ...
        json_str = json.dumps(out, ensure_ascii=False).replace(
            "</script>", "<\\/script>"
        )
        embedded_block = (
